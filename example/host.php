...
<div class="hostinfo">
    ...
    <?php foreach ($sitemaps->sitemaps as $sitemap) { ?>
        <span class="hostinfo_item">
            <?= $sitemap->sitemap_url ?>. Страниц: <?= $sitemap->urls_count ?>. Ошибок: <?= $sitemap->errors_count ?>.
            <?php if (isset($sitemap->last_access_date)) {
                echo "Загружен " . date('d.m.Y', strtotime($sitemap->last_access_date));
            } else {
                echo "Не загружен";
            }
