    ...
    if (is_array($res->hosts) && count($res->hosts) > 0) {
        foreach ($res->hosts as $host) {
            ...
            <li class="hostrow <?=($host->verified)?'verified':'notverified'?>">
                <a href="<?=$url?>"><?=$host->unicode_host_url?></a>
                <a href="./index.php?host_id=<?=$host->host_id?>&delete=true" onclick="return window.confirm('Are you sure you want delete host &laquo;<?=$host->unicode_host_url?>&raquo;?');">[X]</a>
            </li>
            <?php
        }
    } else {
